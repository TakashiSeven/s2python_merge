...
if "%~1"=="" (
    echo Usage: Drag and drop .pak files onto this batch file to check for conflicts
    echo This mode will only analyze conflicts without merging files.
    echo.
    pause
    exit /b 1
)
...
echo This will NOT merge any files - analysis only mode
echo.
...
if %ERRORLEVEL% neq 0 (
    color 0c
    echo.
    echo ERROR: The conflict analysis encountered an error.
    echo.
    pause
    exit /b 1
)
